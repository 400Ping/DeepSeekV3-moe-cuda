...
cudaError_t k4_launch(const Kernel4Problem& p,
    ...
    if (!p.hidden_states || !p.hidden_states_scale || !p.gemm1_weights ||
        !p.gemm1_weights_scale || !p.gemm2_weights || !p.gemm2_weights_scale ||
    ...
    if (!workspace.storage || workspace.storage_bytes < required_workspace ||
        !workspace.gemm1_output || !workspace.gemm2_output || !workspace.output_accum) {
        return cudaErrorInvalidValue;
    }
    ...
    if ((p.backend == Kernel4Backend::Cutlass || p.backend == Kernel4Backend::Auto) &&
        ...
        return kernel4_internal::launch_cutlass_backend(p, workspace, total_tokens);
    }
#endif

    if (p.backend == Kernel4Backend::Tiled) {
        return kernel4_internal::launch_tiled_backend(p, workspace, total_tokens);
    }

    return kernel4_internal::launch_fallback_backend(p, workspace, total_tokens);
}
